...
# === 每小时流量分布（固定模型） ===
ratios = np.array([
    0.065, 0.045, 0.040, 0.030, 0.020, 0.015,
    0.015, 0.020, 0.030, 0.040, 0.050, 0.055,
    0.035, 0.050, 0.045, 0.045, 0.050, 0.045,
    0.045, 0.040, 0.050, 0.060, 0.075, 0.085
])
...
if mode == 1:
    print(f"当前已用：{used_tb:.2f} TB")
else:
    print(f"每日流量：{total_tb:.2f} TB")


if mode == 1 and predicted_today_tb is not None:
    print(f"预测今日流量：{predicted_today_tb:.2f} TB（{mode_label}）")
